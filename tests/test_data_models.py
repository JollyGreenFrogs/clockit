...
def test_task_manager_integration_with_new_time_entry():
    ...
    time_entry = TimeEntry(
        hours=2.5, date=datetime.now(), description="Test time entry with datetime"
    )
    ...
    try:
        ...
        print(
            "✅ TaskManager.add_time_entry_by_id has correct signature for new TimeEntry model"
        )
...
def test_time_entry_datetime_validation():
    ...
    valid_entry = TimeEntry(hours=1.0, date=datetime.now(), description="Valid entry")
    ...
    with pytest.raises(ValidationError):
        TimeEntry(hours=1.0, date="invalid-date-format", description="This should fail")

    print(
        "✅ DateTime validation working correctly - this is the benefit of our refactoring!"
    )
