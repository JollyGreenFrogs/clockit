...
from database.repositories import TaskRepository, CategoryRepository, TimeEntryRepository, ConfigRepository
...
class TestTaskRepository:
    ...
    def test_user_data_isolation(self, task_repo, test_db_session, clean_database):
        ...
        # Create two users
        user1 = User(id=uuid.uuid4(), username="user1", email="user1@test.com", 
                     hashed_password="hash1", is_active=True)
        user2 = User(id=uuid.uuid4(), username="user2", email="user2@test.com", 
                     hashed_password="hash2", is_active=True)
    ...
    def test_delete_task(self, task_repo, test_user_id, clean_database):
        """Test task deletion"""
        # Create a task
        task_repo.create_or_update_task("Task to Delete", time_spent=1.0, user_id=test_user_id)
...
class TestCategoryRepository:
    ...
    def test_get_all_categories_empty(self, category_repo, test_user_id, clean_database):
        """Test getting categories when none exist"""
        categories = category_repo.get_all_categories(user_id=test_user_id)
        assert categories == []
    
    def test_category_user_isolation(self, category_repo, test_db_session, clean_database):
