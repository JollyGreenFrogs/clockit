...
class TestRequestModels:
    ...
    def test_time_entry_model_validation(self):
        ...
        time_entry = TimeEntry(
            hours=8.5, date=datetime.now(), description="Valid work description"
        )
    ...
    def test_time_entry_xss_protection(self):
        ...
        assert (
            "script" not in time_entry.description.lower()
            or time_entry.description.startswith("&lt;")
        )
    ...
    def test_task_create_comprehensive_validation(self):
        ...
        task_trimmed = TaskCreate(
            name="Trimmed Task", category="Development"  # Already trimmed
        )
    ...
    def test_category_create_validation(self):
        ...
        category = CategoryCreate(
            name="Work Category", description="Category for work tasks", color="#FF5733"
        )
        ...
        assert "string_too_long" in str(exc_info.value) or "Invalid hex color" in str(
            exc_info.value
        )
    ...
    def test_onboarding_data_validation(self):
        ...
        valid_categories = [
            cat for cat in onboarding_with_spaces.categories if cat.strip()
        ]
    ...
    def test_currency_config_validation(self):
        ...
        assert "string_too_short" in str(exc_info.value) or "3-letter" in str(
            exc_info.value
        )
...
class TestResponseModels:
    ...
    def test_onboarding_status_response(self):
        ...
        status = OnboardingStatus(
            onboarding_completed=True, default_category="work", needs_onboarding=False
        )
    ...
    def test_success_and_error_responses(self):
        ...
        from data_models.responses import ErrorResponse, SuccessResponse

        success = SuccessResponse(
            message="Operation completed successfully", data={"result": "success"}
        )
        assert success.success is True
        assert success.message == "Operation completed successfully"

        error = ErrorResponse(
            error="Validation failed", details="Invalid input provided"
        )
...
class TestCybersecurityFeatures:
    ...
    def test_input_length_limits(self):
        ...
        with pytest.raises(ValidationError):
            TimeEntry(
                hours=1.0, date=datetime.now(), description="x" * 2000  # Too long
            )

    def test_numerical_bounds_validation(self):
        """Test that numerical inputs have reasonable bounds"""
        from data_models.requests import RateConfig, TimeEntry
    ...
    def test_string_sanitization(self):
        ...
        with pytest.raises(ValidationError):
            TaskCreate(name="Task<script>alert('xss')</script>", category="Development")

        # Test that normal input with sanitization works
        task = TaskCreate(name="Normal Task Name", category="Development")
