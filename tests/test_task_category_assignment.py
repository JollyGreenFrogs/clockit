...
def make_authenticated_request(base_url: str, endpoint: str, token: str, method: str = "GET", data: Dict = None):
...
class TestTaskCategoryAssignment:
    ...
    def test_task_creation_with_category(self):
        ...
        assert created_task["category"] == "Test", f"Expected category 'Test', got '{created_task['category']}'"
    ...
    def test_invoice_generation_with_categorized_tasks(self):
        ...
        response = make_authenticated_request(
            self.BASE_URL, f"/tasks/{test_task['id']}/time", token, "POST", time_entry_data
        )
        print(f"Time entry response: {response.status_code} - {response.text}")
        assert response.status_code == 200, f"Time entry creation failed: {response.text}"
        ...
        assert updated_task["time_spent"] > 0, f"Time not logged properly: {updated_task['time_spent']}"
        
        # Now test invoice preview - this should work now
        response = make_authenticated_request(self.BASE_URL, "/invoice/preview", token, "GET")
        ...
        if invoice_data["status"] != "success":
            print("⚠️  Backend may need restart to pick up invoice manager field mapping fixes")
        ...
        assert invoice_data["status"] == "success", f"Invoice generation failed: {invoice_data}"
        assert "INVOICE PREVIEW" in invoice_data["preview"], "Invoice preview content missing"
        assert "Invoice Test Task" in invoice_data["preview"] or "Test:" in invoice_data["preview"], "Task not found in invoice"
