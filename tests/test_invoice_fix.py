...
def test_invoice_manager_data_structure():
    ...
    # Mock rate manager
    with (
        patch.object(invoice_manager.rate_manager, "load_rates") as mock_rates,
        patch.object(
            invoice_manager.rate_manager, "calculate_hourly_rate"
        ) as mock_hourly_rate,
        patch.object(
            invoice_manager.currency_manager, "get_current_currency"
        ) as mock_currency,
        patch.object(
            invoice_manager.currency_manager, "format_currency"
        ) as mock_format,
    ):
...
def test_backend_preview_endpoint_mock():
    ...
    assert {"preview": preview_text, "status": "success"} is not None
